...
- Analyze notes with a local rule-based analyzer.
...
- Rule-based extraction is keyword-driven and may miss implied decisions or action items.
...
- Speaker labels and timestamps are not currently extracted.
...
- `analyze_rule_based` returns the same logical shape as `analyze_with_openai`: `summary`, `key_decisions`, `action_items`, and `open_questions`.
...
- `build_markdown_report` currently creates a fixed report structure and does not include transcript metadata, source filename, model name, owner fields, or due dates.
